    ...
    return (
      <div className={`markdown-renderer error ${className}`.trim()}>
        <div className='error-message'>
          ...
          {loadingStateData?.error && <p>Details: {loadingStateData.error}</p>}
        </div>
      </div>
    );
  }
          ...
          components={{
            ...
            code: ({ className, children, ...props }) => {
              ...
              if (className && className.includes('language-mermaid')) {
                return (
                  <MermaidDiagram chart={String(children)} className='my-6' />
                );
              }
            ...
            pre: ({ children, ...props }) => (
              <pre
                className='bg-vs-editor-bg p-4 rounded-lg overflow-x-auto mb-4 border border-vs-editor-border'
                {...props}
              >
                {children}
              </pre>
            ...
            blockquote: ({ children, ...props }) => (
              <blockquote
                className='border-l-4 border-vs-editor-border pl-4 italic mb-4 text-vs-editor-text2'
                {...props}
              >
                {children}
              </blockquote>
            ...
            table: ({ children, ...props }) => (
              <div className='overflow-x-auto mb-4'>
                <table
                  className='min-w-full border-collapse border border-vs-editor-border'
                  {...props}
                >
                  {children}
                </table>
            ...
            th: ({ children, ...props }) => (
              <th
                className='border border-vs-editor-border px-4 py-2 bg-vs-editor-surface font-semibold'
                {...props}
              >
                {children}
              </th>
            ...
            td: ({ children, ...props }) => (
              <td
                className='border border-vs-editor-border px-4 py-2'
                {...props}
              >
                {children}
              </td>
            ),
            hr: () => <hr className='border-t border-vs-editor-border my-6' />,
          }}
        >
          {content}
        </ReactMarkdown>
