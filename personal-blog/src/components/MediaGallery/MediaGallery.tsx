...
const MediaGallery: React.FC<MediaGalleryProps> = ({
  ...
  return (
    <div className='mb-6 sm:mb-8'>
      <h3 className='text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-vs-editor-text'>
        Project Media
      </h3>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4'>
        {allMedia.map((media, index) => (
          <div key={index} className='relative group cursor-pointer'>
              ...
              >
                ...
                <div className='absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 rounded-lg'>
                  <div className='w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-white bg-opacity-90 rounded-full flex items-center justify-center'>
                    <svg
                      className='w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-gray-800 ml-0.5 sm:ml-1'
                      fill='currentColor'
                      viewBox='0 0 20 20'
                    >
                      <path
                        fillRule='evenodd'
                        d='M10 18a8 8 0 100-16 8 8 0 000 16zM9.664l-3-2z'
                        clipRule='evenodd'
                      />
                    </svg>
                  </div>
            ...
            {media.caption && (
              <p className='mt-2 text-xs sm:text-sm text-vs-editor-text2 text-center px-1'>
                {media.caption}
              </p>
            )}
          </div>
        ...
        <div className='fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2 sm:p-4'>
          <div className='relative max-w-full max-h-full w-full sm:max-w-2xl lg:max-w-4xl'>
            ...
            {selectedMedia.caption && (
              <p className='mt-2 sm:mt-4 text-white text-center text-sm sm:text-base lg:text-lg px-2'>
                {selectedMedia.caption}
              </p>
            )}
          </div>
