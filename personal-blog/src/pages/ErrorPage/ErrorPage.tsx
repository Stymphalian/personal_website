...
const ErrorPage: React.FC<ErrorPageProps> = ({
    ...
    switch (errorType) {
      ...
      case 'content-error':
        return {
          ...
          message:
            message || 'There was a problem loading the requested content.',
          icon: '📄',
          suggestions: [
  ...
  return (
    <div
      className={`min-h-screen bg-vs-editor-bg flex items-center justify-center px-4 ${className}`}
    >
      <div className='max-w-md w-full bg-vs-editor-surface rounded-lg shadow-lg p-8 text-center'>
        ...
        {/* Error Message */}
        <p className='text-vs-editor-text2 mb-6'>{errorContent.message}</p>

        {/* Error Details (if available) */}
        {errorMessage && errorMessage !== errorContent.message && (
          <div className='bg-vs-editor-surface2 rounded-lg p-4 mb-6 text-left'>
            <p className='text-sm text-vs-editor-text font-medium mb-2'>
              Error Details:
            </p>
        ...
        {/* Suggestions */}
        <div className='mb-8'>
          <h3 className='text-sm font-medium text-vs-editor-text mb-3'>
            Try these solutions:
          </h3>
